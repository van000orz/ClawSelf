...
> 本仓库是一套**基于 OpenClaw 的币安广场数字分身**技能包。所有技能放在 **`skills/`** 下，可直接交给你的 **OpenClaw**（或其它支持 Skill 的 Agent）使用，实现从「灵魂提取 → 热点感知 → 内容生成 → 自动发帖」的全链路。
...
把 **`skills/`** 拷给 OpenClaw 用，或从仓库 **`web/`** 下载单份 Skill、整包 **skills.zip** 均可；优先看各技能目录下的 **README.md** 和技能正文，再按本教程顺序跑即可。
