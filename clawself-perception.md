...
description: |
  ...
  依赖 .cursor/skills/crypto-market-rank 与 .cursor/skills/meme-rush 的 API。适用于「扫一眼市场」「感知层」「热点捕捉」「共识在哪」等指令，或需为分身提供发帖素材时。
metadata:
  version: "1.0"
---
...
- **crypto-market-rank**（`.cursor/skills/crypto-market-rank/SKILL.md`）：Social Hype Leaderboard、Smart Money Inflow Rank 的请求/响应格式。
- **meme-rush**（`.cursor/skills/meme-rush/SKILL.md`）：Topic Rush Rank List 的请求/响应格式。
